...
beta-binomial family here (`phi` controls the overdispersion).
...
`fit_tls()` references columns with tidy evaluation (bare column names, as in `dplyr`).
You pass the success column (`y`, survivors), the trial column (`n`, total), the
duration (`time`) and temperature (`temp`) columns, the family, and the reference time
`tref` at which `CTmax` is defined.
...
If you prefer a `brms`/`drmTMB`-style grammar, build a model with `tls_bf()` and
pass it as the first argument with the data in `data =`. The left-hand side names
the survival counts (`successes | trials(total)`, or `cbind(successes,
failures)`); the right-hand side tags the two axes with the `time()` and `temp()`
markers. Add a sub-parameter formula for any predictor on `CTmax` or `log_z`
(e.g. `CTmax ~ life_stage` for a grouped fit). The formula path feeds the same
likelihood engine as the column call above, so the two fits are numerically
identical:
...
the set of values whose deviance from the maximum stays below the chi-square cutoff,
...
shape (the `bayesTLS` constant-shape configuration):
