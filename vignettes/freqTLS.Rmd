...
minutes. `standardize_data()` converts recognised input duration units to
minutes, so the omitted default is one hour (`t_ref = 60`). A supplied numeric
reference is never converted: `t_ref = 1` deliberately means CTmax at one
minute. Bare formula/column data must already use minutes.
