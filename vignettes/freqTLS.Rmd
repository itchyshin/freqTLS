...
Bayesian [`bayesTLS`](https://github.com/daniel1noble/bayesTLS) package.

This vignette walks through the core loop: simulate → fit → confidence intervals →
plot. It runs end to end with a small simulation.
...
`simulate_tls()` draws survival counts from the known data-generating process: a
...
workflow object. `t_ref` is the reference time at which `CTmax` is defined. `fit_4pl()` uses the `bayesTLS` spelling `t_ref` (default 60), while
...
The reason for the direct `CTmax`/`z` parameterisation is that both become
model *coordinates*, so they can be profiled directly. `confint()` with
