...
(thermal sensitivity, degrees Celsius per decade of exposure duration). It returns
...
the same unit as `duration`; it does not convert units. The `fit_4pl()` facade
uses the `bayesTLS` spelling `t_ref` (default 60), while the lower-level
`fit_tls()` engine uses `tref` (default 1). We declare the simulated durations
to be hours in `standardize_data()`, so `t_ref = 1` defines CTmax at one hour.
