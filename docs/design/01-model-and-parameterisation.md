...
`standardize_data()` converts duration to minutes, and `tref` is always in
minutes, so `CTmax` is the critical thermal maximum at that minute-valued
reference. When omitted, freqTLS uses `tref = 60` (one hour). An explicit
numeric reference is never converted: `tref = 1` is deliberately CTmax at one
minute. Bare formula/column data must already express duration in minutes. When
calling bayesTLS for the benchmark, pin the matching `t_ref`/`time_multiplier`
so the two implementations agree on the unit. The unit and `tref` are surfaced
in `print()`.
