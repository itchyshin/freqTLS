...
| `inst/extdata/case_study_summary_cache.rds` | `data-raw/build_case_study_summary_cache.R`; live freqTLS fits to the shipped shrimp, life-stage zebrafish, and *D. suzukii* datasets | freqTLS maintainers for the compilation; source-data holders retain rights in inputs | Package licence plus the source-specific CC BY 4.0 licences of the three retained datasets | Version-stamped 2026-07-12 build containing 12 headline profile rows, eight contrast rows with the actual profile/bootstrap-fallback method, package/source/R/TMB versions, input checksums, and exact fit configuration | cross-case-study summary vignette and cache-integrity test | **SHIP** with retained dataset attribution |
