...
- The threshold is **absolute** (a fixed 50% survival), following
  Ørsted, rather than the relative midpoint used for the shrimp and
  zebrafish case studies. In `freqTLS` the default `derive_*` behaviour
  at `surv = 0.5` is exactly this absolute LT50, so no extra argument is
  needed here.
...
- `dz:F-M` = **0.054 \[-0.021, 0.128\]** on the log-`z` scale (a `z`
  ratio of `exp(0.054)` ≈ 1.06, i.e. males about 6% steeper), with a
  confidence interval that includes 0 (ratio 1).
