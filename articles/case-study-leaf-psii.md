...
`snowgum_psii` is vendored from `bayesTLS` under CC BY 4.0 (see
[`?snowgum_psii`](https://itchyshin.github.io/freqTLS/reference/snowgum_psii.md)).
Each row is one leaf disc heated at a `(Temp, Time)` combination; the
response `fvfm_prop` is the **retained-PSII proportion** — the
chlorophyll-fluorescence efficiency (Fv/Fm) after the heat dose divided
by its value before. A value near 1 means PSII was essentially
