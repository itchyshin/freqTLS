...
Here `tref = 1` is one hour, and `trace$time` is also in hours. The
trace time, fitted duration, and reference time must use the same unit
because heat-injury damage is accumulated per time unit.
...
broader Bayesian heat-injury and repair workflow; its posterior
intervals are distinct from freqTLS confidence intervals.
