...
#' clearest value-add over the Bayesian path, which has no data-adequacy guard on
#' the model-fitting side (see `docs/design/90-bayesTLS-critique.md`). The twelve
#' warnings in SPEC.md S10 split into two groups:
#'
...
#' data-adequacy concern it finds (SPEC.md S10, items 1-8). Called by [fit_tls()]
...
check_tls_data <- function(y, n, time, temp, group = NULL,
  ...
  if (n_temp < 3L) {
    warn("temps", c(
      ...
      i = "Profile intervals may be wide or fail to close."
    ))
  }
  ...
  if (all(y >= n)) {
    warn("no_mortality", c(
      ...
      i = "CTmax and z cannot be estimated from these data."
    ))
  }
  ...
  if (all(y < n) && all(y > 0)) {
    ...
    if (all(prop > 0.5) || all(prop < 0.5)) {
      warn("threshold", c(
        ...
        i = "Estimates rely on the model's functional form rather than on an observed crossing."
      ))
    }
  }
  ...
  if (!is.null(ctmax)) {
    ...
    if (length(out) > 0L) {
      warn("ctmax_extrapolated", c(
        ...
        i = "CTmax is extrapolated beyond the data; treat it and its interval with caution (R-EXTRAP)."
      ))
    }
  }
  ...
  if (!is.null(phi) && is.finite(phi) && phi > 1e4) {
    warn("phi_binomial_limit", c(
      ...
      i = "The beta-binomial has effectively collapsed to a binomial; consider `family = \"binomial\"` (R-PHI)."
    ))
  }
...
#'
#' `check_tls()` re-runs the data-adequacy diagnostics (SPEC.md S10, items 1-8)
